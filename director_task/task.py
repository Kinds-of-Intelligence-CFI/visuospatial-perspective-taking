...
block_description_options = [
    "Some of the cells in the grid are blocked from the users view hiding anything that might be there indicated by the dark background in the cell.",
    "Some of the cells in the grid are blocked meaning that there is an opaque wall at the back of the cell, this is indicated by the dark background in the cell.",
    "Each grid cell has a backing plate behind the item that is either transparent or made from an opaque grey material.",
    ]

SYSTEM_MESSAGE = """The image displays the front of a shelving grid, which has items stored in grid cells. Your task is to select the correct grid cell that contains the item specified by the user. {block_description} The user is standing on the opposite side of the grid, viewing it from the back. The user has never seen the grid before, and has no prior knowledge as to the location of the items.
...
def directors_task(
    ...
    use_ascii_image: bool = False,
    block_description_index: int = 2,
    ) -> Task:
