...
The stimuli for the followup evaluations are available in the OSF storage in the zip file `follow_up_MWE3.zip` and locked with the password: `follow_up`
...
- `vpt_follow_up.csv` comes from a follow-up experiment using a
  ...
  shipped generator does not produce; its raw results are included here but
  the stimuli themselves cannot be regenerated from this repository.
